...
Getting Started
===============
For ```Hive``` mode you can run this project only with simple R, not in RStudio. Because in ```RStudio#getServer()``` a method ```getURL()``` is called and Hive doesn't support it.
