...
Querying and executing SQL can be done through the usual `DBI` functions, for example:
...
Because of `SqlRender` translation, we can even use non-standard SQL such as `DATEADD`:
...
Because of the many idiosyncrasies in how different dataplatforms store and transform table and field names, it is currently not possible to use any names that would require quotes. So for example the names `person`, `person_id`, and `observation_period` are fine, but `Person ID` and `Obs. Period` are not. In general, it is recommend to use lower case snake_case for database table and field names.
